...
for total=1:10
   ...
   for iteration=1:20
disp(iteration)
cd('C:\Users\User\Desktop\새 폴더\Code\Result\Real\Case2\CSSBL\ESTIMATE');       
filename = sprintf('estimate_%d_%d.csv', total,iteration );
estimate=csvread(filename);       
       
cd('C:\Users\User\Desktop\새 폴더\Code\Result\Real\Case2\CSSBL\X');       
...
cd('C:\Users\User\Desktop\새 폴더\Code\Result\Real\Case2\CSSBL\Z');       
filename = sprintf('z_%d_%d.csv', total,iteration );
z=csvread(filename);  


cd('C:\Users\User\Desktop\새 폴더\Code\Result\Real\Case2\CSSBL\GAMMA');       
filename = sprintf('variance_%d_%d.csv', total,iteration );
variance=csvread(filename);
       
cd('C:\Users\User\Desktop\새 폴더\Code\Result\Real\Case2\CSSBL\FAULT_INDEX');       
filename = sprintf('FAULT_INDEX_%d_%d.csv', total,iteration );
fault_index=csvread(filename);

cd('C:\Users\User\Desktop\새 폴더\Code\Result\Real\Case2\CSSBL\ITERATION')
