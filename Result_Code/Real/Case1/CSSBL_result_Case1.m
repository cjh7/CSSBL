...
for total=1:5
   ...
   for iteration=1:20
disp(iteration)
cd('C:\Users\User\Desktop\새 폴더\Code\Result\Real\Case1\CSSBL\ESTIMATE');       
filename = sprintf('estimate_%d_%d.csv', total,iteration );
estimate=csvread(filename);       
       
cd('C:\Users\User\Desktop\새 폴더\Code\Result\Real\Case1\CSSBL\X');       
...
cd('C:\Users\User\Desktop\새 폴더\Code\Result\Real\Case1\CSSBL\Z');       
filename = sprintf('z_%d_%d.csv', total,iteration );
z=csvread(filename);  


cd('C:\Users\User\Desktop\새 폴더\Code\Result\Real\Case1\CSSBL\GAMMA');       
filename = sprintf('variance_%d_%d.csv', total,iteration );
variance=csvread(filename);
       
cd('C:\Users\User\Desktop\새 폴더\Code\Result\Real\Case1\CSSBL\FAULT_INDEX');       
filename = sprintf('FAULT_INDEX_%d_%d.csv', total,iteration );
fault_index=csvread(filename);


cd('C:\Users\User\Desktop\새 폴더\Code\Result\Real\Case1\CSSBL\ITERATION')
...
label_2(1)=1;
label_2(2)=1;
label_2(fault_index(1)-7)=1;


% Variance Information
var_1=ones(N-7,1)*0.01;
var_1(1)=1;


var_2=ones(N-7,1)*0.01;
var_2(1)=1;
var_2(2)=1;
var_2(fault_index(1)-7)=1;
